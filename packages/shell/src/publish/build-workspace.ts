...
for (;;) {
  try {
    ...
    // Only workspace files are picked (failedWorkspaceFile), so this is a
    // path inside the workspace; the message itself is redacted.
    const relative = path.relative(root, file);
    const lines = redactPaths(vite, root, (error as Error).message, [path.resolve(runtimeSrc)])
      .split("\n")
      .map((line) => line.trim())
      .filter(Boolean);
