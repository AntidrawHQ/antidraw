...
import {
  ANTIDRAW_MCP_SERVER_NAME,
  createAntidrawMcpServer,
} from "@/main/api/tools";
  ...
  try {
    ...
    const res = query({
      ...
      options: {
        ...
        effort,
        mcpServers: {
          [ANTIDRAW_MCP_SERVER_NAME]: createAntidrawMcpServer(workspaceId),
        },
        hooks: onEffortLevel
          ? {
              Stop: [
...
- The workspace's Vite dev server is managed by antidraw. Use the mcp__antidraw__get_dev_server tool to find its status, URL and log file path; never start or stop it yourself. Tail the log file to check for build errors after editing a component.
